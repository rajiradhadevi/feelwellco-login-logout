...
public class DriverHelper {
    ...
    public static boolean AssertPageUrlContains(String s) throws Exception {

        WebDriver driver = AppConfiguration.driver;
        String currUrl = driver.getCurrentUrl();
        boolean result = currUrl.toLowerCase().indexOf(s.toLowerCase()) != -1;
        if(!result) {
            throw new Exception(String.format("%s does not contain %s", currUrl, s));
        }
        return result;
    }
}
